...
use nanogpt::{
    data::TextDataset,
    infer::bigram_lm_infer,
    model::BigramLanguageModelConfig,
    train::{BigramLanguageModelTrainingConfig, bigram_lm_train},
};
use std::fs;
...
fn main() -> Result<(), pico_args::Error> {
    ...
    if args.train {
        let config = BigramLanguageModelTrainingConfig::new(
            BigramLanguageModelConfig::new(dataset.vocabulary.chars),
            AdamWConfig::new(),
        )
    ...
    // Inference
    if args.infer {
        bigram_lm_infer::<MyBackend>(&args.artifact_dir, &device, &args.context, args.max_tokens);
    }
